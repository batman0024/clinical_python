...
# ==== Tabs: Graph (Full) and Ask & Evidence ==================================
tab_graph, tab_qa = st.tabs(["Graph (Full)", "Ask & Evidence"])

with tab_graph:
    st.subheader("Interactive Knowledge Graph (Full)")
    if state.get("kg_json"):
        ...
        # Bigger interactive viewport + no scroll
        st.components.v1.html(html_str, height=900, scrolling=False)
        st.download_button("Download graph.html", data=html_str.encode("utf-8"), file_name="graph.html")
    else:
